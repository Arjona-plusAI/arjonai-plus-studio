/* ============================================
   ...
   Professional Animation System v2.0 (Cleaned)
   ============================================ */
...
const AnimationManager = (function () {
    ...
    function tick(timestamp) {
        ...
        activeAnims.forEach((anim, id) => {
            ...
            const progress = Math.min((elapsed - delay) / anim.duration, 1);
    ...
    return {
        ...
        tween(target, props, options = {}) {
            ...
            return this.animate({
                ...
                onUpdate(t) {
                    Object.keys(props).forEach(key => {
                        target[key] = startVals[key] + (endVals[key] - startVals[key]) * t;
                    });
        ...
        shake(el, intensity = 8, duration = 400) {
            ...
            return this.animate({
                ...
                onUpdate: t => {
                    const osc = Math.sin(t * Math.PI * 7) * intensity * (1 - t);
        ...
        pulse(el, options = {}) {
            ...
            return this.animate({
                ...
                onUpdate: t => {
                    ...
                    el.style.boxShadow = `0 0 ${intensity * osc}px ${color}`;
                }
            });
        },
        ...
        stop(id) { activeAnims.delete(id); },
        get activeCount() { return activeAnims.size; }
    };
})();
...
const UIAnimations = (function () {
    ...
    function getToastContainer() {
        if (!toastContainer) {
            ...
            toastContainer.style.cssText = `
                position: fixed; top: 54px; right: 16px; z-index: 999999;
    ...
    function showToast(message, type = 'info', duration = 3000) {
        ...
        toast.style.cssText = `
            ...
            transform: translateX(40px) scale(0.92); pointer-events: all;
        ...
        close.style.cssText = `flex-shrink:0;cursor:pointer;opacity:0.5;
            font-size:14px;padding:2px 4px;transition:opacity 0.2s;`;
        close.textContent = '✕';
    ...
    function showProgress(label = 'Processing...', color = 'var(--ac)') {
        ...
        if (!overlay) {
            ...
            bar.style.cssText = `
                height: 100%; width: 0%;
                background: linear-gradient(90deg, ${color}, var(--ac2));
    ...
    function createSkeleton(width, height, radius = 8) {
        ...
        el.style.cssText = `
            width: ${width}; height: ${height}; border-radius: ${radius}px;
    ...
    function addRipple(el, color = 'rgba(127,61,255,0.25)') {
        if (!el) return;
    ...
    function initRipples() {
        const selectors = [
            '.tb-btn', '.btn-export', '.btn-primary-full', '.btn-ghost-full',
            '.mob-gen-btn', '.sheet-action-btn', '.sheet-primary-btn',
            '.pixart-btn', '.gp', '.qt-btn', '.bg-rm-btn', '.grade-preset-btn', '.mode-btn'
        ];
        selectors.forEach(sel => {
            document.querySelectorAll(sel).forEach(el => addRipple(el));
        });
    }
    ...
    function transitionPanel(fromEl, toEl, direction = 'left') {
        ...
        setTimeout(() => {
            ...
            requestAnimationFrame(() => {
                ...
                setTimeout(() => { toEl.style.cssText = ''; }, 240);
            });
        }, 180);
    }
    ...
    function animateLogo() {
        ...
        AM.animate({
            ...
            onComplete: () => { logo.style.transform = ''; logo.style.opacity = ''; }
        });
    }
    ...
    function animateSidebar() {
        ...
        AM.animate({
            ...
            onComplete: () => { sb.style.transform = ''; sb.style.opacity = ''; }
        });
    }
    ...
    function animateCanvas() {
        ...
        AM.animate({
            ...
            onComplete: () => { canvas.style.opacity = ''; canvas.style.transform = ''; }
        });
    }
    ...
    function animateBottomPanel() {
        ...
        AM.animate({
            ...
            onComplete: () => { bp.style.transform = ''; bp.style.opacity = ''; }
        });
    }
    ...
    function pulseAIToggle() {
        ...
        AM.animate({
            ...
            onUpdate: t => {
                ...
                btn.style.boxShadow = `0 0 ${10 + osc * 14}px var(--ac), 0 4px 18px var(--glo)`;
            }
        });
    }
        ...
        return AM.animate({
            ...
            onUpdate: t => {
                dots.forEach((dot, i) => {
                    ...
                    dot.style.transform = `translateY(${-5 * bounce}px) scale(${0.8 + 0.4 * bounce})`;
    ...
    return {
        AM, toast: showToast, progress: showProgress, skeleton: createSkeleton,
        ripple: addRipple, initRipples, transitionPanel, canvasFlash,
        animateLogo, animateToolbar, animateSidebar, animateCanvas,
        animateBottomPanel, pulseAIToggle, animateLoadingDots,
        showSuccess, showError, initAll
    };
})();
...
const CanvasAnimations = (function () {
    ...
    function particleBurst(canvas, x, y, color = '#7F3DFF') {
        const particles = [];
        const count = 18;
        ...
        return AM.animate({
            duration: 800, easing: AM.Ease.easeOut,
            onUpdate: t => {
    ...
    function smoothZoomCanvas(canvasEl, fromScale, toScale, duration = 400) {
        return AM.animate({
            duration, easing: AM.Ease.spring,
            onUpdate: t => {
                const s = fromScale + (toScale - fromScale) * t;
                canvasEl.style.transform = `scale(${s})`;
            },
            onComplete: () => { canvasEl.style.transform = ''; }
        });
    }

    return { startSelectionAnim, stopSelectionAnim, particleBurst, smoothZoomCanvas };
})();
...
const MicroInteractions = (function () {
    ...
    function addTilt(el, intensity = 10) {
        ...
        el.addEventListener('mousemove', e => {
            ...
            el.style.transform = `perspective(400px) rotateX(${-rx * intensity}deg) rotateY(${ry * intensity}deg)`;
        });
        el.addEventListener('mouseleave', () => {
    ...
    function initAll() {
        document.querySelectorAll('.help-card, .hist-item').forEach(el => addHoverLift(el, 2));
    ...
    return { addHoverLift, addTilt, addMagnetic, initAll };
})();
...
const ScrollAnimations = (function () {
    ...
    if (!document.getElementById('scrollAnimCSS')) {
        ...
        style.textContent = `
            .scroll-anim { opacity: 0; transform: translateY(20px);
                transition: opacity 0.5s ease, transform 0.5s ease; }
            .anim-fadeUp-done { opacity: 1 !important; transform: translateY(0) !important; }
            .anim-fadeIn-done { opacity: 1 !important; }
            .anim-scaleIn-done { opacity: 1 !important; transform: scale(1) !important; }
        `;
        document.head.appendChild(style);
    }
